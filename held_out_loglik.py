...
idx = train_inds
N_ = len(idx)
(i,j) = list(zip(list(range(0,N_,batch_size)),list(range(batch_size,N_,batch_size))+[N_]))[0]
batch_idx = idx[i:j]
...
model.load_weights('model_fits/{}_{}_{}.h5'.format(K,fold,seed))

f = open('heldout/heldout_logliks_{}_{}.txt'.format(K,seed),'a')

for i in test_inds:
    X = [lang_id[i:i+1],pos_id[i:i+1],encoder_input[i:i+1],decoder_input[i:i+1]]
    y = tf.one_hot(decoder_output[i:i+1],n_output+1)[:,:,1:]
    log_p_z,pred_out = model(X)
    losses_z = log_p_z + tf.reduce_sum(pred_out*tf.expand_dims(y,-3),[-1,-2])
    loss_value = -tf.reduce_mean(tf.reduce_logsumexp(losses_z,-1))
    print(loss_value.numpy(),file=f)
    print(i)
